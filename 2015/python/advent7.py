...
class instruction:
    def __init__(self, line):
        self.operation = []
        parsed_operation = parse_operation( line )
        self.operation.append( parsed_operation.group(1) )
        self.operation.append( parsed_operation.group(2) )
        self.operation.append( parsed_operation.group(3) )
...
def parse_operation( line ):
    match = re.search(r'([a-z0-9]+)? ?(NOT|AND|LSHIFT|RSHIFT|OR)? ?(\w+)? -> (\w+)', line)
